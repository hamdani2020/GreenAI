...
# Load TTS and STT pipelines (cached)
@st.cache_resource
def load_tts_pipeline():
    import numpy as np
    import torch
    speaker_embeddings = np.load("speaker_embedding.npy")
    speaker_embeddings = torch.tensor(speaker_embeddings).unsqueeze(0)  # Add batch dimension
    synthesiser = pipeline("text-to-speech", "microsoft/speecht5_tts")
    def generate_speech(text):
        return synthesiser(text, forward_params={"speaker_embeddings": speaker_embeddings})
...
def send_gemini_request(
    ...
    if image_base64:
        ...
        payload["contents"].append({"parts": [{"text": prompt}]})
        ...
        with conversation_container:
            for message in st.session_state.conversation_history:
                if message["role"] == "user":
                    ...
                    tts_result = tts_pipeline(message["content"])
                    audio_array = tts_result["audio"]
                    sample_rate = tts_result["sampling_rate"] if "sampling_rate" in tts_result else 16000
